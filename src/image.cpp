...
void Image::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, uint32_t mipLevels, VkSampleCountFlagBits numSamples) {
	...
	vkGetImageMemoryRequirements(device.getDevice(), image, &memRequirements); // image
	...
	vkBindImageMemory(device.getDevice(), image, imageMemory, 0); // image
}
